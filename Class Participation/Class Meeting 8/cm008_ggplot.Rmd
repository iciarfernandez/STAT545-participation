---
title: "Class Meeting 8 Exercises: Fix the Plots"
output: 
  html_document:
    keep_md: true
    theme: paper
...
ggplot(mpg, aes(cty, hwy)) +
  ...
  geom_smooth(method = "lm") + #it's fitting the line to the original points, not to the jitter - the fact that the jitter is there doesn't mean that it changes the original data. so, it's using the cty and hwy columns to plot the regression line, not the jitter points 
  theme_bw()
```
...
gapminder %>% 
  ...
  geom_line(alpha = 1/3) +
  scale_colour_discrete("", labels = c("Other", "Rwanda")) #first argument is the title of the aesthetic, labels = tells you which labels there are, but it is kind of redundant to have the title be Rwanda so to have a blank title for the legend you just need ""
```
