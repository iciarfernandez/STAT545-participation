...
```{r}
gapminder %>% 
  group_by(country) %>% #either group_by or arrange could go first
  arrange(year) %>% #by default it arranges in ascending order
...
gapminder %>%
  ...
  arrange(inc_life_exp) %>% #arrange function works independently of groups
  knitr::kable()

#gapminder dataframe only has 5 year differences, that is why this code produces the 5 year difference rather than a 1 year difference
