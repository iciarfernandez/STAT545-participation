...
```r
time %>% 
  semi_join(album, by = c("song", "artist_name")) # artist_name is added because there could be 2 songs with the same name made by diff artists
```
...
```r
time %>% # Want to preserve everything in time, add album info if there is album info but if not would rather see NA
  inner_join(album, by = c("song", "artist_name"))
```
...
```r
time %>% 
  anti_join(album, by = "artist_name") # anti-join shows what is in "time" that is not in "album" (i.e. subtracts) So Mariah Carey is in the time tibble but not in the album tibble 
```
...
```r
fell %>% 
  anti_join(ttow, by = "Race") %>% #Removes everything that doesn't appear in ttow
  anti_join(retk, by = "Race") #0 rows because there are no races present in FELL that aren't in the other ones
```
...
```r
anti_join(z, y) #If we were to have different variables in either z or y you'd have to use anti_join, not setdiff
```
